  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        if (!res.ok) {
          throw new Error(data.error || 'Google Signup failed on server');
        }
  ...
  return (
    ...
    >
      <div className="absolute top-8 right-8 text-sm text-[#a1a1aa]">
        Already a user? <Link href="/login" className="text-white underline underline-offset-4 hover:text-yellow-500 transition-colors">Login</Link>
      </div>
